...
export const POST = async ({ request }) => {
    try {
        ...
        if (!GEMINI_API_KEY) {
            throw new Error("API Key არ არის კონფიგურირებული .env ფაილში");
        }

        // 1. ვიპოვოთ რომელი მოდელია ხელმისაწვდომი
        ...
        if (!matchedDocs || matchedDocs.length === 0) {
            return json({ text: "ბაზაში ამ საკითხზე ოფიციალური ინფორმაცია არ მოიძებნა." });
        }
